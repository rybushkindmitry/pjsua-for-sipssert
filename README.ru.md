...
Режимы `uac` и `uas` поддерживают все 3 транспорта (TLS, TCP, UDP) через параметр `--transport`.
Режимы с TLS-ролями (`uas-tls-client`, `uac-tls-server`) всегда используют TLS независимо от `--transport`.
...
ruri_user: test       # user-часть Request-URI
from_user: uac        # user-часть заголовка From
...
reinvite_delay: "3"   # задержка(и) в секундах, через запятую: "3" или "3,7,12"
options_ping: 5       # интервал отправки in-dialog OPTIONS, сек
options_auto_reply: true   # только отвечать на OPTIONS (без отправки)
...
Exit code `0` требует выполнения **всех** условий:
- **Звонок завершён** — вызов установлен и корректно завершён
- **Echo-валидация** — match rate RTP ≥ `--tolerance` (по умолчанию 90%), или медиа не ожидалось
- **Проверка заголовков** — все `--expect-header*` проверки пройдены (если настроены)
- **OPTIONS ping** — процент успешных ≥ `--options-tolerance` (по умолчанию 90%), если настроен `--options-ping`

При невыполнении любого условия или ошибке соединения — exit code `1`.
...
| `--transport` | `tls`, `tcp`, `udp` (для uac/uas; tls-роли используют TLS) | `tls` |
| `--proxy HOST:PORT` | Адрес удалённой стороны | - |
| `--port PORT` | Локальный SIP/TLS-порт | 5060 / 5061 |
| `--bind-ip ADDR` | Привязать к конкретному IP / интерфейсу | 0.0.0.0 |
...
| `--ruri-user USER` | User-часть Request-URI | `test` |
| `--from-user USER` | User-часть заголовка From | `uac` / `uas` |
...
Обе стороны независимо генерируют детерминированные аудио-фреймы через `EchoValidatorPort`. Каждый фрейм заполняется одним постоянным PCM-значением из таблицы **μ-law-стабильных значений** (диапазон 500–20000). Значение циклически меняется для каждого нового фрейма:

```
Фрейм 0: все 160 samples = -508
Фрейм 1: все 160 samples =  508
Фрейм 2: все 160 samples = -524
...
```

Обе стороны используют один и тот же алгоритм генерации, начиная с sequence 0, поэтому производят идентичные фреймы.

**Путь данных:**

```
EchoValidatorPort → conference bridge → G.711 encode → RTP → сеть →
→ RTP → G.711 decode → conference bridge → EchoValidatorPort
```

μ-law-стабильные значения проходят через G.711 PCMU encode/decode без потерь. Однако conference bridge PJSUA2 ослабляет амплитуду примерно в 3–12×.

**Сравнение на приёме:**

1. Вычисляется **медиана** всех samples в полученном фрейме
2. Если `|медиана| < 10` — фрейм считается тишиной (mismatch)
3. Поиск в ring buffer (64 последних отправленных значения): совпадение **знака** и амплитуды в пропорциональном диапазоне (`|sent| / 12 ≤ |медиана| ≤ |sent|`)

**Совместимость с кодеками:**

| Кодек | Статус | Примечание |
|---|---|---|
| PCMU (G.711 μ-law) | Проверено, ~98% match | Значения — фиксированные точки μ-law |
| PCMA (G.711 A-law) | Ожидается работа | Аналогичное сохранение знака/амплитуды |
| Speex, Opus, G.722 | Ожидается работа | Может потребоваться расширение допуска |
| Кодеки с DTX/подавлением тишины | Возможны проблемы | Constant-fill фреймы могут быть приняты за тишину |

**Практические замечания:**
- `--tolerance=0` отключает валидацию (проверяется только установка вызова)
- `--tolerance=90` (по умолчанию) — рекомендуется для тестов pjsua↔pjsua и через OpenSIPS
- Первые 2–3 фрейма могут не совпасть из-за задержки установки вызова (ring buffer ещё пуст)
- При неожиданно низком match rate проверьте наличие обработки медиа в пути (AGC, noise gate, транскодинг)

### Управление BYE

| Параметр | Описание | По умолчанию |
|---|---|---|
| `--bye` | `uac`, `uas`, `none` — кто отправляет BYE | Зависит от режима |
| `--wait-bye` | Таймаут ожидания BYE от удалённой стороны, сек | 30 |

По умолчанию BYE отправляет та сторона, которая владеет вызовом (`uac` для uac/uac-tls-server, `uas` для uas/uas-tls-client). При `--bye=none` никто не отправляет BYE — вызов живёт `--duration` секунд, после чего скрипт завершается с результатом echo-валидации.

Если `--bye` указывает на другую сторону, скрипт ждёт BYE от удалённой стороны не более `--wait-bye` секунд после истечения `--duration`. Если BYE не получен — exit 1.

### re-INVITE

| Параметр | Описание | По умолчанию |
|---|---|---|
| `--reinvite-by` | `uac` или `uas` — кто отправляет re-INVITE | - |
| `--reinvite-delay` | Задержка(и) после установления медиа, через запятую (сек) | - |

Оба параметра обязательны вместе. Пример: `--reinvite-by=uac --reinvite-delay=3` отправит re-INVITE через 3 секунды. Множественные re-INVITE: `--reinvite-delay=3,7,12`.

После re-INVITE `EchoValidatorPort` переподключается к новому медиа-объекту — счётчики не сбрасываются, валидация продолжается.

Обе стороны могут быть инициаторами (каждая указывает `--reinvite-by` для своей роли). Разносите таймеры во избежание glare (SIP 491).

### In-dialog OPTIONS ping

| Параметр | Описание | По умолчанию |
|---|---|---|
| `--options-ping` | Интервал отправки OPTIONS, сек (включает auto-reply) | - |
| `--options-auto-reply` | Только отвечать 200 OK на входящие OPTIONS | false |
| `--options-tolerance` | Минимальный % успешных ответов на OPTIONS | 90 |

Сторона-инициатор (`--options-ping=N`) отправляет in-dialog OPTIONS каждые N секунд и проверяет процент полученных 200 OK ответов. Другая сторона (`--options-auto-reply`) только отвечает.

Результат OPTIONS-валидации влияет на exit code наравне с echo-валидацией и проверкой заголовков.
...
├── scripts/
│   ├── common.py           # Общий модуль: EchoValidatorPort, OptionsPingManager,
...
│   ├── pjsua-bye-none/        # без BYE (обе стороны --bye=none)
